...
def import_labels_to_table(path, origin, fps):
    """import labels and convert into shape used in table""
    :return annotation_series, unique_labels
    """
    annotation = load_labels(path, origin=origin, fps=fps)
    unique_labels = list(annotation.columns)
    # remove time column if it's not an index already
    if "time" in unique_labels:
        unique_labels.remove("time")

    # convert annotations from dummies to series
    annotations = pd.from_dummies(annotation.drop(columns=["time"]), default_category= "other")
